...
#########################################################
# This code will use the 'pathcoverage.tsv' file generated by HUMAnN 3.0 to determine the average pathway coverage among samples
...
path.abd <- read.delim('D://ERIN_humann_pathabundance_relabd_merged.tsv', header=TRUE, sep='\t')
path.cov <- read.delim('D://ERIN_pathcoverage_genus_merged.tsv', header=TRUE, sep='\t')
metadata <- read.csv('D://ERIN_MetaboliteAnalysis_Metadata_CaseFollowPairs_clean.csv',header=TRUE)
...
# overlook...) I am going to hold off on filtering based on pathway coverage and filter based on abundance (see below). 
# However, here is code designed to perform filtering based on the average coverage estimate per pathway across all 118 samples.
...
write.csv(path.abd.cutoff, 'D://ERIN_pathabundance_relabd_merged_CaseFollowPairs.csv', row.names=FALSE)
